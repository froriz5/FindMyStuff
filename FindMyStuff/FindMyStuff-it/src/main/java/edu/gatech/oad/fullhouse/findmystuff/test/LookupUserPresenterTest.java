...
public class LookupUserPresenterTest extends
    ...
    private void assertErrorVisibility(int vis) {
        assertEquals(vis, getActivity().findViewById(R.id.notFoundError).getVisibility());
    }

    private void assertUserDisplayed(User testUser) {
        assertEquals(testUser.getUsername(), ((TextView)getActivity().findViewById(R.id.usernameDisplay)).getText());
        assertEquals(testUser.getName(),     ((TextView)getActivity().findViewById(R.id.nameDisplay)).getText());
        assertEquals(testUser.getEmail(),    ((TextView)getActivity().findViewById(R.id.emailDisplay)).getText());
    }
	...
	public void testLookupUserNotFound() throws Throwable {
	    ...
	    final LookupUserPresenter presenter = new LookupUserPresenter(activity, new TestUserAccessor(testUser));
